...
class UiServer {
  ...
  updateRoutes(routes) {
    ...
    if (oldRoutes !== JSON.stringify(routes)) {
      ...
      const removedRoutes = oldRoutesArray.filter(oldRoute =>
        !routes.some(route =>
      ...
      if (newRoutes.length > 0) {
        newRoutes.forEach(route => {
          ...
          this.logChange(`Service ${route.serviceName} added route ${route.incomingHost} ${destination} ${route.target.href}`);
        });
      }
      ...
      if (removedRoutes.length > 0) {
        removedRoutes.forEach(route => {
          const destination = route.type === 'proxy' ? '->' : '=>';
          this.logChange(`Service ${route.serviceName} removed route ${route.incomingHost} ${destination} ${route.target.href}`);
        });
      }
  ...
  async start() {
    const templateContent = await readFile(join(__dirname, 'template.eta'), 'utf8');
    ...
    server.listen(8080, () => {
      console.log('UI server listening on port 8080');
    });
