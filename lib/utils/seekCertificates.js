...
async function getDomainsFromFile (path) {
  ...
  const pemCerts = data.match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/g);
...
async function getDomainsFromCert (pemCert) {
  const { stdout } = await execPromise(`echo '${pemCert}' | openssl x509 -noout -text`);
  const commonNameRegex = /Subject:.*? CN\s*=\s*([^\s,]+)/;
  const altNameRegex = /X509v3 Subject Alternative Name:\s*((?:\s+[^\s,]+(?:,\s+)?)+)/;
