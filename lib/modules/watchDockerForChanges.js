...
function watchDockerForChanges (router, stoppingContainers = new Set(), uiServer) {
  return new Promise((resolve) => {
    ...
    function parseDockerEvent (response) {
      ...
      if (response.Type === 'container') {
        ...
        console.log(`Docker event: ${response.Type} ${action} for ${containerName} (${containerId}), image: ${imageName}, service: ${serviceName}`);
        uiServer?.logChange(`Container ${containerName} ${action}`);

        // Define events that should trigger immediate route removal
        const stopEvents = ['die', 'kill', 'stop', 'pause', 'destroy', 'remove'];

        // Immediately remove routes for containers that should not receive traffic
        if (stopEvents.includes(action)) {
          console.log(`Container ${containerName} (${containerId}) is ${action}, removing routes immediately`);

          // Add this container to the stopping containers set
          if (containerId) {
            stoppingContainers.add(containerId);
            console.log(`Added ${containerId} to stopping containers set (size: ${stoppingContainers.size})`);
          }
          if (containerName && containerName !== 'unknown') {
            stoppingContainers.add(containerName);
            console.log(`Added ${containerName} to stopping containers set (size: ${stoppingContainers.size})`);
          }
          if (serviceName && serviceName !== containerName) {
            stoppingContainers.add(serviceName);
            console.log(`Added ${serviceName} to stopping containers set (size: ${stoppingContainers.size})`);
          }
          ...
          if (!removedById && containerName && containerName !== 'unknown') {
            console.log(`Also trying to remove routes by container name: ${containerName}`);
            ...
            if (!removedByName && serviceName && serviceName !== containerName) {
              console.log(`Also trying to remove routes by service name: ${serviceName}`);
          ...
          if (containerId) {
            ...
            console.log(`Removed ${containerId} from stopping containers set (size: ${stoppingContainers.size})`);
          }
          if (containerName && containerName !== 'unknown') {
            stoppingContainers.delete(containerName);
            console.log(`Removed ${containerName} from stopping containers set (size: ${stoppingContainers.size})`);
          }
          if (serviceName && serviceName !== containerName) {
            stoppingContainers.delete(serviceName);
            console.log(`Removed ${serviceName} from stopping containers set (size: ${stoppingContainers.size})`);
          }

          // Refresh all routes
          console.log(`Container ${containerName} (${containerId}) is ${action}, refreshing routes`);
          listRoutesFromServicesFiltered(router, stoppingContainers);
        } else {
          // For other container events, still refresh all routes to keep everything in sync
          console.log(`Container ${containerName} (${containerId}) had event ${action}, refreshing routes`);
          listRoutesFromServicesFiltered(router, stoppingContainers);
        }
        ...
        console.log(`Docker service event: ${action} for ${serviceName} (${serviceId})`);
        ...
        if (['remove', 'delete'].includes(action)) {
          console.log(`Service ${serviceName} (${serviceId}) is ${action}, removing routes immediately`);

          // Add to stopping containers set
          if (serviceId) {
            stoppingContainers.add(serviceId);
            console.log(`Added ${serviceId} to stopping containers set (size: ${stoppingContainers.size})`);
          }
    ...
    function removeContainerRoutes(containerId) {
      ...
      console.log(`Checking ${currentRoutes.length} routes for container ID ${containerId}`);
      ...
      const updatedRoutes = currentRoutes.filter(route => {
        ...
        const nameInServiceId = containerName &&
                               route.serviceId.includes(containerName);

        // Check if the service name is part of the route's serviceId
        // This handles docker-compose naming patterns
        const serviceNameMatches = serviceName &&
                                  route.serviceId.includes(serviceName);
        ...
        const serviceIdContainsName = containerName &&
                                     containerName.includes(route.serviceId);

        // If any match is found, we should remove this route
        const shouldRemove = idMatches || nameInServiceId || serviceIdContainsName || serviceNameMatches;

        if (shouldRemove) {
          console.log(`Removing route: ${route.configValue} for service ${route.serviceId}`);
          console.log(`Match reason: ID match: ${idMatches}, Name in ID: ${nameInServiceId}, ` +
      ...
      if (updatedRoutes.length !== currentRoutes.length) {
        console.log(`Removed ${currentRoutes.length - updatedRoutes.length} routes for container ${containerId}`);
        ...
        if (containerNameParts.length > 1) {
          console.log(`Trying to match by container name parts: ${containerNameParts.join(', ')}`);
          ...
          const updatedRoutesByName = currentRoutes.filter(route => {
            ...
            for (const part of containerNameParts) {
              if (part && part.length > 3 && route.serviceId.includes(part)) {
                console.log(`Found match by name part "${part}" for route: ${route.configValue}`);
                shouldKeep = false;
                break;
              }
            }
          ...
          if (updatedRoutesByName.length !== currentRoutes.length) {
            console.log(`Removed ${currentRoutes.length - updatedRoutesByName.length} routes by name pattern matching`);
