...
function createProxy (routes, request, response) {
  ...
  try {
    ...
    route = routes.find(route => url.match(route.incomingHost));
  } catch (error) {
    console.log(error);
  }
...
function createProxyServer (routes, certificates) {
  ...
  const server = https.createServer({
    SNICallback: (serverName, callback) => {
      ...
      const matchingCertificateKey = Object.keys(key => {
        return key === serverName || `*.${serverName}`;
      });
